...
#'
#' This is a wrapper function streamlining hierarchical clustering of Hi-C
#' contact matrices into topologically-associating domains(TADs). It uses hclust
#' and dendrogram cutting to predict TADs and sub-TADs.
#'
...
#' @param cluster_method a string indicating the agglomeration method to be used
#' for hierarchical clustering. It is "ward.D2" by default, but can also be set
#' to "ward.D", "single", "complete", "average" or "centroid".
#' @param plot_subtads A boolean denoting whether predicted sub-TADs should be
#' plotted individually
#' @param dend_labels a string one of "none", "perpindicular" or "textlike" for
#' no, vertical, or horizontal labels, respectively.
#'
#' @return plt_dendrogram a graphics.plot object storing cluster-annotated dendrogam.
#'
#' @export
#'
#' @examples
#' data("mESChromInteractions")
#' data.cl <- cluster(mESChromInteractions, offset=40000)
#'
...
cluster <-
    function(hsa,
             ...
             plot_subtads = FALSE,
             dend_labels = "none") {
        # check paramaters are provided
        if (missing(hsa) | missing(offset)) {
            stop("cluster needs both HSA and ffset arguments")
        }

        # agglomerative clustering with hsa
        k <- floor(dim(hsa)[1] / 2) - 1 # max number of clusters
        hsa_scaled <- scale(hsa)
        hsa.d <- stats::dist(hsa_scaled, method = "euclidean")
        hsa.cl <- stats::hclust(hsa.d, method = cluster_method)

        # display to user clustering parameters
        print(hsa.cl)

        # convert clustered groups to dendrogram for cutting
        ...
        if (dend_labels != "none")
            dendrogram <-
            dendextend::color_labels(dendrogram, k = k)
...
#'
#' Finds and groups sub-TADs and TADs into hierarchical clusters, with the
#' option of the plotting them separatelyy.
#'
...
#' \item \emph{hcluster}: full hierarchical subclusters of the TAD given the
#' flat_cluster
#' }
#'
...
tad.group <- function(dendrogram, tads, k, plot_subtads, offset) {
    ...
    # plots predicted hierarchical clusters individually
    if (plot_subtads)
        sapply(subtads, graphics::plot)

    # flatten list to get a unique path name from TAD to sub-TAD
    labelled_tads <- names(LinearizeNestedList(subtads))
    labelled_tads <-
        stringr::str_replace_all(labelled_tads, "/", "-")
    labelled_tads <-
        stringr::str_replace(labelled_tads, "^[0-9]*", as.character(tads))

    # calculate beginning and end of TADs
